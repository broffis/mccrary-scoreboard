...
const GroupPlayer = (props) => {
  ...
  return (
    <div className="group-player group__grid">
      ...
      { eventScores.map(event => <p className="group-player__score-box u-justify-self-center font-secondary-semibold" key={`${props.player_id}-${event.event_name}`} style={boxStyles}><span className="group-player__score font-secondary-semibold">{event.points ? event.points : '-'}</span></p>)}
    </div>
  )
};
